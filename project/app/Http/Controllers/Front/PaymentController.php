 ...
 public function store(Request $request){
    ...
    // Redirect to paypal IP
     ...
     public function payreturn(){
         return redirect()->back()->with('success','Payment Completed. You Have Successfully Bought This Auction');
     }
...
// Set TCP timeout to 30 seconds
    ...
    if (strcmp($res, "VERIFIED") == 0 || strcasecmp($res, "VERIFIED") == 0) {
        ...
        $auction = Auction::find($order->auction_id);
              $data = Bid::where('user_id','=',$order->user_id)->where('auction_id','=',$order->auction_id)->first();
                if(isset($data)){
                    $data->bid_amount = $order->pay_amount;
                    $data->winner = 1;
                    $data->update();
                    $notf = new Notification();
                    $notf->order_id = $order->auction_id;
                    $notf->save();
                    ...
                    $input['auction_id'] = $order->auction_id;
                    $input['bid_amount'] = $order->pay_amount;
                    $input['winner'] = 1;
