...
class DashboardController extends Controller
{
    ...
    {   
        $opened = Auth::user()->auctions()->where('status','=',1)->get()->count();
        $closed = Auth::user()->auctions()->where('status','=',0)->get()->count();
        return view('user.dashboard',compact('opened','closed'));
    }
    ...
    public function my_profileupdate(Request $request)
    {
        //--- Validation Section
